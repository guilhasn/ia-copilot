---
title: "Início"
layout: home
nav_order: 1
---

# Inteligência Artificial

## Aplicações ao trabalho das IES

Esta formação foi desenhada para funcionários técnicos, administrativos, especialistas e de gestão de instituições de ensino superior.

O objetivo é capacitar os formandos para utilizar ferramentas de Inteligência Artificial, com especial foco no Microsoft 365 Copilot, em tarefas reais do trabalho das IES: comunicação institucional, análise documental, tratamento de informação, apoio à decisão, serviços académicos, qualidade, contratação pública, gestão de reuniões e produção de conteúdos.

O curso combina demonstrações práticas, exercícios guiados, casos de uso do contexto universitário e reflexão crítica sobre os limites da IA em processos sensíveis.

{: .note }
> **Acesso aos materiais**
> Alguns materiais descarregáveis podem estar protegidos por password.
> Caso seja solicitada password, utilize a password comunicada pela ANFUP.

---

## Programa das sessões

| Sessão | Bloco | Tema | Estado |
|---:|---|---|---|
| 1 | Enquadramento | Usar IA com critério no trabalho das IES | Preparada |
| 2 | Enquadramento | Do pedido vago ao prompt eficaz | Preparada |
| 3 | Enquadramento | Biblioteca pessoal de prompts | A preparar |
| 4 | Produtividade | Word com Copilot | A preparar |
| 5 | Produtividade | Outlook e Teams | A preparar |
| 6 | Produtividade | Excel com Copilot | A preparar |
| 7 | Produtividade | PowerPoint com Copilot | A preparar |
| 8 | Processos universitários | Serviços académicos, pedidos e regulamentos | A preparar |
| 9 | Processos universitários | Atas e reuniões dos órgãos | A preparar |
| 10 | Processos universitários | Contratação pública | A preparar |
| 11 | Processos universitários | Júris e candidaturas | A preparar |
| 12 | Processos universitários | Qualidade, acreditação e prestação de contas | A preparar |
| 13 | Automatização | Copilot Pages e bibliotecas partilhadas | A preparar |
| 14 | Automatização | Literacia sobre agentes Copilot Studio | A preparar |
| 15 | Governança | Plano de adoção pessoal | A preparar |

---

## Contactos

- **Entidade promotora:** [ANFUP — Associação Nacional dos Funcionários Universitários Portugueses](https://www.anfup.pt/)
- **E-mail:** anfup.formacao@gmail.com
- **Formador:** Nuno Salvador
